...
const ImageContainer = ({ id, src, name, description, category, price }: { id: string, src: string, name: string, description: string, category: string, price: number | undefined }) => {
  ...
  return (
    <div className="bg-[#eeeeee] p-6 rounded-lg shadow-lg flex flex-col justify-between card">
      <div className="mb-4">
      ...
      <div className="flex justify-between">
        <div>
          <Link to={`/product/${id}`} className="text-white bg-[#ce9d48] rounded-md px-4 py-2 transition-all duration-300 hover:bg-gray-800 text-center mr-2">
            Shop Now
          </Link>
        </div>
      </div>
